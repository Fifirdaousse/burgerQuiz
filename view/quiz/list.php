    ...
    <?php foreach ($args['listQuiz'] as $key => $question) : ?>
        <div class="border-ra light-blue text-center p-5 m-b-35 ">
            ...
            <div class="border-ra  bgc-w m-15 black">
                <input type="radio" name="proposition<?= $key ?>" 
                id="proposition<?= $key . '-' . $keyProp ?>" 
                value="<?= $proposition['estBonneReponse'] ?>">
                <label for="proposition<?= $key ?>"><?= $proposition['lettre'] . ' - ' . $proposition['intitule'] ?>
            </label>
            </div>
            <?php endforeach; ?>
        </div>
